...
# 演示和验收用的那件事,一个串,两处共用(demo-record.sh 和 tests/test-cart.sh)。
# 测试跑的要是和演示跑的不是同一件事,它通过了也说明不了演示能过。
#
# 任务按**主人平时说话的样子**写:说清要什么、不要什么,不写步骤。
# 把「打开淘宝 → 搜索 → 点第一个 → 加购」列成清单,演示的就只是「它会照着念」;
# 而真实的委托从来是「你看着办,办完告诉我」。所以判断留在任务里(挑哪个由它
# 比完再定),理由要在结论里(让它说出为什么是这个)—— 视频最后一屏于是有两样
# 东西可以互相对照:购物车里多出来的那件实物,和它自己给的挑选依据。
#
# 条件写成**排除项 + 一条够用就行的线**,不写成「尽量挑最 X 的」。
# 「尽量挑销量高、店铺正规的」量过一轮:29 步没加成。它没有做错任何一步 ——
# 那句话没有上界,任何一刻都还能有更高销量的,于是它一路排序、筛选、滚动、再筛选。
# 第 17 步它手上已经有一个站得住的候选(¥1398,100+ 付款,店铺正常),丢下它去点
# 销量排序,再没回来。真人嘴上说「挑个靠谱的」,心里带着「差不多就行」,
# 这半句不写出来,agent 就真的会一直挑下去。
#
# 这里原先写的是「把排在最前面的那一个加入购物车,不用纠结它标题写的是几代」。
# 那是拿措辞去绕开模型的一个毛病(它在「按位置取第一个」和「按名字对得上」之间
# 反复横跳),题目一换措辞就失效。根因已经在提示词里治掉(要它先比再挑),
# 措辞不必再背这个包袱 —— 注意「第一个**满足条件的**」和原来那个「第一个商品」
# 不是一回事:前者要求先拿条件筛一遍再定,后者是按位置取。
export DEMO_GOAL="淘宝上帮我挑一副 AirPods Pro 2 加进购物车。二手、资源机、美版这些别要,正经店铺卖的全新的就行,第一个满足的就下手,别挑花眼。加完告诉我挑的是哪个、为什么是它。"
