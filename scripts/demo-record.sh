...
# 措辞是量出来的,别改回「第一个商品」。真机上「第一个」有歧义:淘宝搜 AirPods Pro 2
# 排第一的标题是「…AirPods PRO3代…2代4代国行」,模型在「按位置取第一个」和
# 「按名字匹配」之间反复横跳 —— 点进详情页、觉得不对、back、回到搜索页看到还是它、
# 再点进去,实测连着四步都在这个圈里,最后靠卡死判定收工。把「最前面的那一个」
# 说死之后同一条链路 9 步 40 秒做完。
GOAL="${1:-打开淘宝,搜索 AirPods Pro 2,把搜索结果里排在最前面的那一个商品加入购物车。不用纠结它标题写的是几代,位置最靠前的那个就是目标。}"
FROM="${FROM:-adb}"
