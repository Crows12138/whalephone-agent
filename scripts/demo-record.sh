...
# 演示任务的串在 scripts/lib.sh 的 DEMO_GOAL(和加购测试共用一个,理由写在那里)。
# 换一件事演示就直接给参数:
#
#   bash scripts/demo-record.sh "淘宝上找把红轴机械键盘,300 以内挑个口碑好的加购物车"
#   bash scripts/demo-record.sh "淘宝和京东各查一下 AirPods Pro 2 多少钱,哪边便宜告诉我"   # 跨 App,步骤翻倍,真机上没跑够轮次
#
# 别把任务写成步骤清单。清单只能演示「它会照着念」,而这道题要证明的是它自己会办事。
GOAL="${1:-$DEMO_GOAL}"
FROM="${FROM:-adb}"
