...
# 顺序很重要,而且改过一次。
#
# 让路挡的是**每一个**动作 —— 试过只挡 launch,真机上不成立:点一下淘宝搜索栏
# 会新开一个 Activity,在窗口管理器眼里和 am start 是同一件事,照样收键盘,
# 而 agent 事先不可能知道哪一次点击会开新窗口(见 Agent.kt 那段 A/B 记录)。
#
# 于是测试的时序也得跟着变:原来是「等 agent 动起来再叫键盘」,那时第一个 launch
# 已经过去了,让路一次都不触发,读数全 0 但什么也没测到。现在副屏已经在(第一段
# 造好的),所以先把键盘叫起来再下任务,第一个动作正好撞上让路。
