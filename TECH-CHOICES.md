...
- **无障碍服务开着时,微信不让付款。** 真机上撞到的:微信收银台报「存在安全风险」并拒绝付款。
  ...
  和盗刷木马没有区别,和本项目做了什么无关。处理办法是关掉服务(`settings delete secure
  enabled_accessibility_services`),但那样 agent 的眼和手也同时没了 —— 两者只能二选一。
  (关掉之后付款是否恢复,机主没有回报,我没有验证过。)
  合理的产品形态是让用户能一键停用 agent;把这一条藏起来不写,评审人自己撞上会更糟。
