...
| 全局焦点 | 副屏上只要出现可获焦窗口(造屏、启 App、页面跳转),**主屏当场丢掉自己的焦点窗口** —— 机主的软键盘被收起,他的 App 报「应用未响应」 | 事前避让:机主在打字时不做动作(`Conflict.yieldWhileOwnerTypes`;判据是键盘弹着**且**他的输入框最近在变,少了后半句他把键盘留着走开 agent 就永远不干活);副屏跨任务复用,把造屏这一下压到一个会话一次 |
