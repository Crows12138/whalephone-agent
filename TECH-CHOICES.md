...
但**自己不调还不够**。副屏上的 App 会自己去要:点开淘宝的搜索页,它给输入框自动
对焦,那个输入框向系统要输入法,键盘照样弹在机主眼前(机主自己发现的,一秒冒出来
一秒落回去)。所以还要给副屏设 `DISPLAY_IME_POLICY_HIDE` —— 这块屏永远不显示输入法,
把 App 那条路也堵掉。副屏上的输入框该对焦对焦,只是不再有人替它把键盘弹到别人脸上。
...
| 输入法 | 整机一个 IME,`mDisplayIdToShowIme` 恒为 0。自己不调还不够 —— **副屏上的 App 会自己去要**(点开淘宝搜索页,输入框自动对焦),键盘照样弹到机主脸上 | 文字走 `ACTION_SET_TEXT` 永不调输入法;再给副屏设 `DISPLAY_IME_POLICY_HIDE`,把 App 那条路也堵掉 |
| 音频(**已知未处理**) | 音频也是全机一份。副屏上的淘宝商品页自动播视频,机主什么都没干手机开始出声 —— 实测 `state:started` 的 AudioTrack 来自副屏上那个 App | 暂缓。方案是启动前 `appops PLAY_AUDIO ignore`、收工还原(要带崩溃收尾),机主判断这条可以接受 |
| 导航键 | `performGlobalAction` 没有显示器维度,会让用户的 App 后退一页 | 返回走 `input -d <屏> keyevent 4`;HOME 那条按键对副屏无效,改用显式启动该屏的 `category.HOME` |
| 全局焦点 | 副屏上只要出现可获焦窗口(造屏、启 App、页面跳转),机主的软键盘被收起,他的 App 报「应用未响应」 | 事前避让:机主在打字时不做动作(`Conflict.yieldWhileOwnerTypes`);副屏跨任务复用,把造屏这一下压到一个会话一次 |
...
### 「机主在打字」这个判断本身,是整套避让里最难的一块

「全局焦点」那一行的处理是「机主在打字时不做动作」。这句话要成立,先得答出
**什么叫在打字**,而这个判断栽过两次,两次的方向正好相反。

只看**状态**(主屏上有输入法窗口)—— 机主把键盘留在屏上走开,agent 就永远不干活。
补上**活动**(最近有键落下)之后,又栽在中文输入上:拼音打进去了、词还没选,
输入框一个字都不变(讯飞连 `setComposingText` 都不调),盯着候选栏想的那十几秒
一条事件都没有,10 秒空闲一到判据就判他停手 —— 副屏一造出来,机主没上屏的拼音
被当场提交(真机 3/3,截图里那串拼音直接被当成搜索发了出去)。

「正在组词」这个状态从系统外面读不到,三条路都验过:IMMS 的组词区间恒为 `-1`、
输入法窗口的无障碍树只有 1 个节点、窗口几何组词前后完全一致。所以只能换判法:

- 活动检测器看「**最后一次动的时刻**」,不看「5 秒内动了几次」。实测打一个字母
  只发约 2 条事件,频率阈值等于要求他打得够快 —— 而打得快不快和停没停手无关。
- 空闲阈值**分两档**,按「这一次键盘弹起期间他有没有按过键」分:按过键 60 秒
  (`DRAFT_IDLE_MS`,他手里可能攥着没上屏的字),一个键都没按过 10 秒(`TYPING_IDLE_MS`)。

60 秒和「活动信号全哑掉时的让路上限」取的是同一个数,这样无论信号可不可用,
一个已经开始打字的机主至少都有 60 秒安静时间。**代价是他打完字把手机放下、键盘
还开着,agent 会白等 60 秒** —— 有意偏向等待:等待是可见的(写进这一步的结果和
日志),打断不可逆。
