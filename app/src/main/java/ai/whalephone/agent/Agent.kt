...
class Agent(
    ...
    fun run(): Outcome {
        ...
        while (n < maxSteps && rounds < maxSteps * 4) {
            ...
            if (idled) {
                ...
                trace[trace.lastIndex] = trace.last().let {
                    it.copy(result = it.result + when {
                        v == null   -> "  ← 这一步之后我看不出界面变化(只是看不出,不代表没生效)"
                        moved       -> "  ← 元素列表没变,但画面变了:" +
                            v.removePrefix(CHANGED).trimStart(':', ':', ' ')
                        else        -> "  ← 这一步之后我看不出界面变化,前后两帧也看不出区别" +
                            "(只是看不出,不代表没生效)"
                    })
                }
            } else sameCount = 0
            ...
            // 看图这一路上给的图是**标了号的**(Set-of-Mark):每个可点元素画一个框,
            // 角上写它在元素列表里的序号。
            //
            // 补的是一处具体的断裂:淘宝的规格弹层里那些按钮在无障碍树里**有节点、
            // 有精确坐标,却没有任何文字** —— 列表里是一排「[12] View "—" 可点」。
            // 模型看得见图上的「确定」,也看得见列表里的号,两边就是对不上;
            // 对不上它就放弃列表去猜像素,实测猜偏率约一半(点到屏幕顶端 y=57,
            // 而确认按钮在 y=958)。标号一画,它读号就行,坐标由系统从节点里取。
            val mk = if (eyes != null) marks(snap) else emptyList()
            val shot = eyes?.let { hands.frameB64(mk) }
            ...
            val act = parse(reply) ?: run {
                ...
                // 不告诉它就等于让它照原样再说一遍。实测连着三轮解析不了,三轮全白烧,
                // 而它历史里干干净净,看不出哪儿不对。
                record(n, "", "?", "你上一轮输出的不是一个能解析的 JSON 对象 —— " +
                    "只输出一个 JSON 对象,动作参数照动作表补齐,前后不要有别的文字。")
                null
            } ?: continue
    ...
    /**
     * 要画到图上的元素。只画点得动的 —— 全画上去满屏是框,反而挡住界面本身,
     * 而模型要在图上认的恰恰是界面。
     */
    private fun marks(snap: Perception.Snapshot): List<Pair<Int, android.graphics.Rect>> {
        val area = snap.elements.maxOfOrNull { it.bounds.width().toLong() * it.bounds.height() }
            ?.coerceAtLeast(1L) ?: return emptyList()
        return snap.elements
            .filter { it.clickable || it.editable }
            // 铺满整屏的容器不画。它的框会包住整幅图、标号落在左上角,而那个角上
            // 通常是别的东西 —— 模型会把这个号读成那个东西的号。实测淘宝的规格
            // 弹层就只有这么一个「可点」元素,画出来纯是噪声。
            .filter { it.bounds.width().toLong() * it.bounds.height() < area * 6 / 10 }
            .filter { it.bounds.width() > 12 && it.bounds.height() > 12 }
            .take(MAX_MARKS)
            .map { it.index to it.bounds }
    }
    ...
    companion object {
        ...
        /**
         * 随图给出的那句话,说的是**这一张图**的事实:标号是画上去的,不是每屏都有。
         *
         * 原来它是个常量,而提示词里写死了「图上每一个点得动的元素都用红框圈出来了」。
         * 淘宝的规格弹层整层自绘,无障碍树里只剩三个没有名字的容器,一个号都画不出来
         * —— 图上没有号,提示词却说有,模型就去找号:实测连着四轮输出 `click` 不带
         * 序号,任务停在离完成一步的地方。图上有什么,只有这里知道,就得由这里说。
         */
        private fun eyesNote(marks: Int) = if (marks > 0)
            "(这一屏的无障碍树读不出文字,所以给你的是截图。图上 $marks 个能定位到的元素" +
                "已经用红框标了号,认准了读号用 click。)"
        else
            "(这一屏的无障碍树连一个能定位的元素都读不出来,所以给你的是**一张没有任何" +
                "标号**的截图。这一屏 click 没有号可用,只能用 tap 报归一化坐标。)"
        ...
        private val SYSTEM_EYES = """
            ...
            凡是还能从无障碍树里定位到的元素,都在图上用红框圈了出来,框的左上角写着
            它的**序号** —— 认准了读号,用 `click` 点号。**这样点是精确的**:坐标由系统
            从元素本身取,不是你估的。

            **但不是每一屏都有号。** 整页自绘的界面里一个元素都定位不到,那张图上就一个
            框都没有。这一屏到底有没有号,看每一轮开头括号里那句话 —— 它说的是**这张图**
            的事实,比你的印象可靠。没有号的时候只能用 `tap` 报坐标,坐标是**归一化**的:
            横竖都是 0 到 1000,(0,0) 左上角,(1000,1000) 右下角,不要给像素值。

            每一步只输出一个 JSON 对象,前后不要有别的文字:
            {"thought":"一句话说明这一步为什么","action":"动作名", ...动作参数}

            可用动作:
              click  i(点图上标着 i 号的那个元素。图上有号就用它,精确)
              tap    x, y(图上没有号的时候用,给元素中心的归一化坐标)
            ...
            - 号是画在元素**左上角**的。看清楚那个号属于哪个框,别读成旁边元素的号。
            - 「界面没有变化」是**我这边看不出变化**,不是「你的动作没生效」。有的操作
              成功之后只闪一下提示就没了,有的结果根本不在这一屏上。这条只说明**再点
              一次也看不出名堂**:换个位置、换条路,或者去能查到结果的地方核对。
            - **你的动作弹出来的东西,是这一步的下半截,不是挡路的东西。** 规格选择、
              确认框、权限询问都是这一类:它们出现恰恰说明你点对了,而**这一步还没生效**。
              在它上面把这一步做完 —— 它通常有一个和你刚点的那个同名的按钮,点那个才算数。
              关掉它等于放弃这一步,而界面会退回你点之前的样子,看起来就像「刚才没反应」,
              于是你会再点一次、再关一次,一直转圈。
            - **会改变外部状态的动作**(加入购物车、提交、发送、确认)做一次,然后**下一步
              ...
              核对的时候认**留得住**的痕迹:列表里多出来的一行、角标上的数字、变成已选中的
        ...
        private val SYSTEM = """
            ...
            - 「界面没有变化」是**我这边看不出变化**,不是「你的动作没生效」。有的操作
              成功之后只闪一下提示就没了,有的结果根本不在这一屏上。这条只说明**再点
              一次也看不出名堂**:换个元素、换条路,或者去能查到结果的地方核对。
            - **你的动作弹出来的东西,是这一步的下半截,不是挡路的东西。** 规格选择、
              确认框、权限询问都是这一类:它们出现恰恰说明你点对了,而**这一步还没生效**。
              在它上面把这一步做完 —— 它通常有一个和你刚点的那个同名的按钮,点那个才算数。
              关掉它等于放弃这一步,而界面会退回你点之前的样子,看起来就像「刚才没反应」,
