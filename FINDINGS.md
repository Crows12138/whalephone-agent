...
| | 只看状态(旧) | 状态 + 活动(新) |
|---|---|---|
| 机主留着键盘走开 | 每轮让满 60 秒,永远不干活 | 停手 20 秒后开工 |
...
于是判据会拿到一个「读得到、且一直没变」的指纹 —— 判成机主已经停手,20 秒后动手,
...
20 秒(`TYPING_IDLE_MS` 可调)。20 秒里输入框一动不动,更像人走开了而不是在措辞。
...
(日志里「输入框已经 513 毫秒没动」—— 不是等到 20 秒阈值,是键盘没了)。
