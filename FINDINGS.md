...
1. **检测器一从频率改成时间戳** —— 看「输入法窗口最后一次动的时刻」,不看「5 秒内动了几次」。
   慢慢打也认得出来。
2. **空闲阈值分两档**,按「这一次键盘弹起期间机主有没有按过键」分:
   - 一个键都没按过 → 10 秒(`TYPING_IDLE_MS`)。输入框自动获焦、键盘自己弹出来的居多,
     他手里没有没上屏的东西,判错只是键盘收一下。
   - 按过键 → 60 秒(`DRAFT_IDLE_MS`)。他手里可能攥着一串没上屏的字。

60 秒不是拍的:活动信号万一在某台机器上全哑掉,判据会退回只看状态,那条路的让路上限
本来就是 60 秒。取同一个数,「信号可用」和「信号不可用」两种情况下,一个已经开始打字的
机主至少都有 60 秒安静时间,不会因为某条检测器失灵而变短。

**代价说清楚:机主打了几个字就把手机放下、键盘还开着,agent 会白等 60 秒。**
这是有意偏向等待 —— 等待是可见的(写进这一步的结果和日志),打断不可逆。
不满意可以调 `DRAFT_IDLE_MS`。

## 改完的真机读数

| 情形 | 改之前 | 改之后 |
|---|---|---|
| 打拼音不提交,停手 | 让 10.4 秒,键盘被收、拼音被提交(3/3) | 让 60.2 秒,键盘保住(3/3) |
| 慢速打字(3 秒一个字母,共 42 秒) | 频率凑不够,中途放行 | 每个字母都认到,让了 102.6 秒 |
| 键盘开着但一个键没按 | 10 秒放行 | 10 秒放行(不变) |

`scripts/tests/test-idle-keyboard.sh` 从 8 条断言扩到 14 条,新增 B(短档)和 C(只打拼音
不上屏)两个用例,14/14 通过。C 这个用例是这次真正缺的那个 —— 原来的用例每次打字都
带空格提交,测的全是英文式输入,中文用户真正会遇到的那一种一次都没测过。
