...
- **`am broadcast --es` 的值里有空格,会被设备端的 shell 再切一次。**
  `--es goal "打开计算器,算 13 乘以 4"` 送到 app 手里只剩「打开计算器,算」。
  现象极具误导性:模型看起来在莫名其妙地反复问你要参数,像是它变笨了或者
  ask 的阈值被调坏了 —— 我照着这个假象改过一轮提示词。用 `lib.sh` 里的 `bc`
  (它在外面又套了一层单引号),或者自己写成 `--es goal "'...'"`。
...
- **别用盲点击驱动悬浮球。** 语音面板等不到回答 6 秒后会自己收起(这是产品行为,
  ...
  **但球的坐标不必靠猜。** `uiautomator dump` 看不见悬浮窗(它只导出当前活动
  窗口),这就是当初只能盲点的原因。`dumpsys window windows` 能看见,而且给的是
  精确像素:找 `Window{... u0 ai.whalephone.agent}` 那一节,读它的 `frame=`。

  ```
  Frames: parent=[0,92][1080,2340] ... frame=[917,1379][1063,1525]
  ```

  中心点就是 (990, 1452)。面板展开后同一节的 frame 会跟着变,面板里那些按钮
  仍然只能靠截图定位 —— 所以上面那条「不要回来看图」照旧,只是第一下不用赌了。

**装完 APK 无障碍会被关掉,而重新打开的两条 setting 有写入顺序。** One UI 上
`adb install` 之后 `enabled_accessibility_services` 里我们的服务会消失,表现是任务
一发就 FAIL「无障碍服务没开」。重新写回去的时候:

```
先写服务列表、再写 accessibility_enabled   →  列表被系统回滚,白写
先写 accessibility_enabled=1、再写服务列表  →  留得住
```

顺序反了不报错,只是悄悄没生效 —— 所以写完必须读回来核对。`scripts/ensure-a11y.sh`
封了这件事,`demo-record.sh` 的体检里直接调它(这条没有第二种正确状态,不提醒、直接修)。
